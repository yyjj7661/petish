...
function commentList(param) {
	...
	$.ajax({
		url:'/dog/gatherboard/comment/commentList',
		type:'GET',
		data : params,
...
$('#input_data').click(function(event){
	...
	jQuery.ajax({
		url : '/dog/gatherboard/comment/insertComment',
		type : 'POST',
		data : params,
		contentType:'application/x-www-form-urlencoded; charset=UTF-8',
		dataType : "json",
...
function getComment(id) {
	...
	jQuery.ajax({
		url : '/dog/gatherboard/comment/'+id,
		type : 'GET',
		contentType:'application/x-www-form-urlencoded; charset=UTF-8',
		dataType : "json",
		success : function(result) {
...
function modifyComment(id, callback, error) {
	...
	$.ajax({
		type : 'put',
		url : '/dog/gatherboard/comment/modifyComment/' + id,
...
function removeComment(id) {
	...
	if(confirm("삭제하시겠습니까?")) {		
		$.ajax({
			type : 'put',
			url : '/dog/gatherboard/comment/removeComment/' + id,
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr) {
				if(result) {
					//alert('댓글삭제!');
